// Mocks generated by Mockito 5.4.4 from annotations
// in flutter_task_manager_app/test/data/repositories/todo_repository_impl_test.dart.
// Do not manually edit this file.

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'dart:async' as _i4;

import 'package:flutter_task_manager_app/src/data/datasource/task_manager_local_datasource.dart'
    as _i3;
import 'package:flutter_task_manager_app/src/data/models/task.dart' as _i2;
import 'package:mockito/mockito.dart' as _i1;

// ignore_for_file: type=lint
// ignore_for_file: avoid_redundant_argument_values
// ignore_for_file: avoid_setters_without_getters
// ignore_for_file: comment_references
// ignore_for_file: deprecated_member_use
// ignore_for_file: deprecated_member_use_from_same_package
// ignore_for_file: implementation_imports
// ignore_for_file: invalid_use_of_visible_for_testing_member
// ignore_for_file: prefer_const_constructors
// ignore_for_file: unnecessary_parenthesis
// ignore_for_file: camel_case_types
// ignore_for_file: subtype_of_sealed_class

class _FakeTaskModel_0 extends _i1.SmartFake implements _i2.TaskModel {
  _FakeTaskModel_0(
    Object parent,
    Invocation parentInvocation,
  ) : super(
          parent,
          parentInvocation,
        );
}

/// A class which mocks [TaskManagerLocalDataSource].
///
/// See the documentation for Mockito's code generation for more information.
class MockTaskManagerLocalDataSource extends _i1.Mock
    implements _i3.TaskManagerLocalDataSource {
  MockTaskManagerLocalDataSource() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i4.Future<List<_i2.TaskModel>> fetch() => (super.noSuchMethod(
        Invocation.method(
          #fetch,
          [],
        ),
        returnValue: _i4.Future<List<_i2.TaskModel>>.value(<_i2.TaskModel>[]),
      ) as _i4.Future<List<_i2.TaskModel>>);

  @override
  _i4.Future<_i2.TaskModel> add(_i2.TaskModel? task) => (super.noSuchMethod(
        Invocation.method(
          #add,
          [task],
        ),
        returnValue: _i4.Future<_i2.TaskModel>.value(_FakeTaskModel_0(
          this,
          Invocation.method(
            #add,
            [task],
          ),
        )),
      ) as _i4.Future<_i2.TaskModel>);


  @override
  _i4.Future<int?> update(_i2.TaskModel? task) => (super.noSuchMethod(
        Invocation.method(
          #update,
          [task],
        ),
        returnValue: _i4.Future<int?>.value(),
      ) as _i4.Future<int?>);
}
